...
│   - lee el árbol de      │   POST /action → tap/swipe/   │   percibe → decide (Claude) │
...
- **`agent/`** — CLI en Node/TypeScript que recibe una tarea en lenguaje natural, y en bucle: pide el árbol de UI, se lo pasa a Claude junto con la tarea y el historial, ejecuta la acción que decida el modelo (usando tool calling), y repite hasta que el propio modelo llama a `finish`.
...
- Node.js 18+ y una `ANTHROPIC_API_KEY` para el agente.
